...
3.  **In-Memory Cache**: Keeps the token in memory to avoid repetitive login requests.
4.  **Singleton Pattern**: Ensures the same instance is reused, preserving the token cache even during _hot reloads_ in development environments.
...
- **Execution Flow**:
  ...
  2.  Checks if the token is still valid by comparing the current time with `this.expiresAt`.
  3.  Applies a safety margin of **60 seconds**: if there is less than a minute left before expiration, it is considered invalid to avoid failures during the request.
...
- **Execution Flow**:
  ...
  5.  Uses the `jwt-decode` library to read the token _payload_ and get the expiration date (`exp`).
  6.  If the token doesn't have an `exp` field, sets a default expiration time of 1 hour (3600 seconds) and logs a warning.
  7.  Updates the `token` and `expiresAt` properties.

### Error Handling

The `refreshAccessToken` method is wrapped in a `try-catch` block. In case of request or decoding failure:

- A critical error is recorded via `logger.error`.
...
- `API_CLIENT_SECRET`: Client secret/password (mapped to `password` in the POST body).

## Integration with `apiFetch`

`TokenService` is mainly consumed by the `apiFetch` utility (`src/lib/api-client.ts`). This integration has a security mechanism:

1.  **Automatic Acquisition**: Before each request, `apiFetch` calls `tokenService.getToken()` to include the `Authorization: Bearer <token>` header.
