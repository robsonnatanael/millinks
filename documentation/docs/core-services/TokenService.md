...
4.  **Singleton Pattern**: Ensures the same instance is reused, preserving the token cache even during *hot reloads* in development environments.
...
-   **Execution Flow**:
    ...
    5.  Uses the `jwt-decode` library to read the token *payload* and get the expiration date (`exp`).
