...
A classe `TokenService` é responsável pelo gerenciamento centralizado de tokens de autenticação JWT para comunicação _Machine-to-Machine_ (M2M) com a API backend. Ela implementa o padrão Singleton para garantir uma única instância do serviço em toda a aplicação e utiliza estratégias de cache para otimizar o desempenho e reduzir chamadas desnecessárias à API de autenticação.
...
4.  **Padrão Singleton**: Garante que a mesma instância seja reutilizada, preservando o cache do token mesmo durante _hot reloads_ em ambiente de desenvolvimento.
...
- **Fluxo de Execução**:
  ...
  5.  Utiliza a biblioteca `jwt-decode` para ler o _payload_ do token e obter a data de expiração (`exp`).
